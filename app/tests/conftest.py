...
def db_init_products(database):
    ...
    for index_name in range(3):
        product_obj = Product(
            name=f"test_sale_product{index_name}", price=99.98, type="sale"
        )
...
def db_init_products_dict(db_init_products):
    return [
        {"product_id": product.id, "qty": product.id} for product in db_init_products
    ]
...
def raw_recipe_hot_dog(path_mock_delay):
    return [
        {"ingredient": "box m", "operation": None, "time": None},
...
@pytest.fixture
def product_dict():
    return {"id": 1, "name": "test_sale_product0", "price": 99.98, "type": "sale"}
