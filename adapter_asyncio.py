...
def main(product_id, qty):
    ...
    print ("\n{} \t Product: {} will be cooked ".format(time.perf_counter() - start, product_obj.name))
    print ("{} \t Real qty {} ({} iterations) ".format(time.perf_counter() - start,
           iterations * CLUSTER_QTY,
           iterations))

    for receipt_item in product_obj.receipts:
        print("\t{}\t{}\t{}".format(receipt_item.ingredient, receipt_item.operation, receipt_item.wait_time))

    for iter_index in range(iterations):
        iteration(product_id)
        print("Iteration {} was finished".format(iter_index))
