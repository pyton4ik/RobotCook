...
async def test_create_product_order(monkeypatch, db_init_products, aiohttp_client, order_dict, db_init_products_dict):
    response = await aiohttp_client.post("/order/", json={"name": "aiohttp_test_order",
                                                          "ref": "aiohttp_555",
...
async def test_cook_order(monkeypatch, aiohttp_client, order_dict, route):
    order_id = 2
