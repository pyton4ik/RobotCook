...
@pytest.mark.parametrize("wrong_recipe_item,reises_class", [(("ketchup", None , None), NotReadyForCooking),
...
def test_wrong_recipie_(monkeypatch, wrong_recipe_item, reises_class):
    if wrong_recipe_item[0] == "ketchup": monkeypatch.setattr(SouceDosatorController, "qty", 0)
